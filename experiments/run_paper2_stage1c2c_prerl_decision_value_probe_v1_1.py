#!/usr/bin/env python
"""P2-1C-2C-v1.1: smoke-only pre-RL matched-budget sequential probe.
...
OUT = BASE / "p2_1c_2c_prerl_decision_value_probe_v1_1"
FROZEN = {
...
class UnsupportedPerceptionQuery(RuntimeError):
    """Only a frozen sampler Unsupported query, with its first-call context."""
...
def query(frozen, emulator, L, year, trajectory, day):
    ...
    try:
        ...
        if not str(exc).startswith("Unsupported query "):
            raise
        # Read the same frozen candidate set, including for local-date failures.
        # No sampling retry, support widening, or fallback is performed.
        raise UnsupportedPerceptionQuery(str(exc), {
            "year": year, "first_unsupported_L": float(L),
            "local_rows": int(len(emulator._candidate_indices(float(L)))),
            "trajectory": trajectory, "day": day,
        }) from exc
...
def calibrate_smoke(frozen, emulator, asset, policy, history=None):
    ...
    Unsupported proposals lower the upper bound, with no fallback or widening.
    """
    ...
    for iteration in range(6):
        ...
        proposal = {"year": asset["year"], "policy": policy,
                    "iteration": iteration, "threshold": threshold,
                    "status": "PENDING", "mean_clean_count": None,
                    "budget_residual": None, "absolute_budget_residual": None,
                    "relative_target_error": None, "first_unsupported_L": None,
                    "local_rows": None, "trajectory": None, "day": None}
        history.append(proposal)
        try:
            result = rollout(frozen, emulator, asset, policy, threshold)
        except UnsupportedPerceptionQuery as exc:
            proposal.update(exc.context)
            ...
            raise
        # Only a complete finite rollout can enter the count-only ranking.
        arrays = ("pre", "post", "raw_next")
        if policy != "True-State":
            arrays += ("q50", "width")
        if (len(asset["calendar"]) != 365
                or result["pre"].shape != (SMOKE_N, 365)
                or not all(np.isfinite(result[key]).all() for key in arrays)):
            proposal["status"] = "ERROR"
            raise RuntimeError("Calibration requires a finite full 365-day rollout")
        count = float(result["actions"].sum(axis=1).mean())
        error = abs(count - SMOKE_BUDGET)
        proposal.update({"status": "SUPPORTED",
        ...
        if count > SMOKE_BUDGET:
            ...
            high = threshold
    if best is None:
        raise RuntimeError(f"No supported candidate after 6 proposals: {asset['year']} {policy}")
...
def main():
    ...
    manifest = {
        ...
        "predecessor_stage": "P2-1C-2C-v1", "predecessor_status": "FAIL",
        "repair_reason": "support-unaware threshold proposal caused unsupported online perception query",
    ...
    summary = {"stage": STAGE, "mode": "smoke", "gates": gates,
               ...
               "unsupported_proposal_count": 0, "unsupported_proposals": [],
               "selected_thresholds_all_supported": False}
    rows = []
    try:
        ...
        for year, seed in DEV_ENV_ROOTS.items():
            ...
            for policy in POLICIES:
                history = []
                calibration = {"year": year, "policy": policy,
                               "selected_threshold_supported": False,
                               "proposals": history}
                # Attach before execution so even an all-unsupported failure is logged.
                summary["budget_calibration"].append(calibration)
                threshold, result, history = calibrate_smoke(frozen, emulator, asset, policy, history)
                results[policy] = result
                count = float(result["actions"].sum(axis=1).mean())
                calibration.update({"selected_threshold_supported": True,
    ...
    summary["unsupported_proposals"] = [
        proposal for calibration in summary["budget_calibration"]
        for proposal in calibration["proposals"] if proposal["status"] == "UNSUPPORTED"]
    summary["unsupported_proposal_count"] = len(summary["unsupported_proposals"])
    summary["selected_thresholds_all_supported"] = bool(
        len(summary["budget_calibration"]) == len(DEV_ENV_ROOTS) * len(POLICIES)
        and all(c["selected_threshold_supported"] for c in summary["budget_calibration"]))
    ...
    try:
        ...
        summary["stage_pass"] = bool(all(gates.values()) and not summary["errors"]
                                     and summary["selected_thresholds_all_supported"])
