...
out_dir = os.path.join(
    project_root, "outputs", "mc_conformal_resnet50_with_i", "figures_final_v2"
)
...
def draw_metric_comparison(axs):
    ...
    panels = [
        ...
        ("Spearman $\\rho$", "Spearman", (0.00, 0.36), "{:.3f}"),
    ]
...
def plot_metric_comparison():
    ...
    save_fig(fig, "fig1_metric_comparison_final_v2")
...
def plot_mc_conformal_scatter(ax=None, sample_n=110, random_state=42, show_legend=True):
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig2_mc_conformal_scatter_final_v2")


# ============================================================
# Figure C: width-error relationship, V2
# ============================================================
...
def binned_stats(x, y, n_bins=8, min_count=25):
    """
    Return median width, median error, q25, q75 by quantile bins.
    This makes Fig. 3 much cleaner than raw scatter only.
    """
    x = np.asarray(x)
    y = np.asarray(y)
...
def plot_width_error_relationship(ax=None, sample_n=700, random_state=42):
    own_fig = ax is None
    ...
    # Display clipping only affects visualization, not metrics.
    y_max_display = 0.30
    ...
    # Binned median + IQR bands
    xs_s, med_s, q25_s, q75_s = binned_stats(
    ...
    xs_m, med_m, q25_m, q75_m = binned_stats(
    ...
    if len(xs_s) > 1:
        ...
        xs_s, med_s, q25_s, q75_s = xs_s[order], med_s[order], q25_s[order], q75_s[order]

        ax.fill_between(xs_s, q25_s, q75_s, color=C_SPLIT, alpha=0.10, linewidth=0)
        ax.plot(
            xs_s,
            med_s,
            color=C_SPLIT,
    ...
    if len(xs_m) > 1:
        order = np.argsort(xs_m)
        xs_m, med_m, q25_m, q75_m = xs_m[order], med_m[order], q25_m[order], q75_m[order]

        ax.fill_between(xs_m, q25_m, q75_m, color=C_MC_CONF, alpha=0.12, linewidth=0)
        ax.plot(
            xs_m,
            med_m,
            color=C_MC_CONF,
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig3_width_error_relationship_final_v2")
...
def plot_lbin_coverage_compare(ax=None):
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig4_lbin_coverage_compare_final_v2")
...
def plot_final_multipanel():
    ...
    plot_width_error_relationship(ax=ax_c, sample_n=700)
    ...
    png_path = os.path.join(out_dir, "fig5_mc_conformal_multipanel_final_v2.png")
    pdf_path = os.path.join(out_dir, "fig5_mc_conformal_multipanel_final_v2.pdf")
    svg_path = os.path.join(out_dir, "fig5_mc_conformal_multipanel_final_v2.svg")
...
if __name__ == "__main__":
    ...
    print("\nFinal V2 publication-style figures saved to:")
    print(out_dir)