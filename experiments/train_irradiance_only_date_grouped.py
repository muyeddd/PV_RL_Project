"""Leakage-free date-grouped irradiance-only training entry point."""
...
from utils.date_grouped_training_data import FORBIDDEN_MODEL_ROLES
...
DEFAULT_CONFIG_PATH = (
    PROJECT_ROOT / "configs" / "training" / "irradiance_only_date_grouped_v1.json"
)
...
MODEL_NAME = "IrradianceOnlyMLP"
INPUT_PREPROCESSING = {
    "source": "raw irradiance parsed from selected model_development filenames",
    "dtype": "float32",
    "standardized": False,
    "clipped": False,
    "strict_unit_interval_assumed": False,
    "physical_unit": None,
}
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train irradiance-only MLP using the frozen date_grouped_v1 manifest"
    )
...
def preflight_fold(
    ...
    train_records, validation_records = load_irradiance_only_fold_records(
        manifest_path, fold
    )
...
def unpack_batch(batch):
    if len(batch) < 2:
        raise ValueError("Expected loss label and irradiance")
...
def run_epoch(
    ...
    for batch_index, batch in enumerate(loader, start=1):
        labels, irradiance = unpack_batch(batch)
        ...
        with torch.set_grad_enabled(is_training):
            with torch.amp.autocast(
                ...
                if predictions.shape != labels.shape:
                    raise RuntimeError(
                        "Prediction/label shape mismatch would cause broadcasting: "
...
def _pretrained_provenance() -> dict[str, Any]:
    return {
        ...
        "pretrained_not_applicable": True,
    }
...
def run_training(args: argparse.Namespace) -> dict[str, Any]:
    ...
    preflight = preflight_fold(args.manifest, args.fold)
    ...
    validation_records = preflight["validation_records"]
    train_dataset = IrradianceOnlyDataset(train_records["filename"].tolist())
    validation_dataset = IrradianceOnlyDataset(
        validation_records["filename"].tolist()
    )
    ...
    pretrained = _pretrained_provenance()
    model = IrradianceOnlyMLP(dropout=0.3).to(device)
