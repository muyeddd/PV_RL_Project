...
def unpack_batch(batch):
    """
    兼容两种情况：
    - dataset 返回 (image, L, I)
    - dataset 返回 (image, L, I, time_feat)
    """
    if len(batch) == 3:
        images, L, I = batch
    else:
...
def update_ema_model(ema_model, model, decay=0.999):
    with torch.no_grad():
        ...
        # buffer（如 BatchNorm 的 running_mean / running_var）直接同步
...
# 工具函数
# ======================
def evaluate(eval_model, loader, criterion, device):
    eval_model.eval()
    total_loss = 0.0
    preds = []
    targets = []
...
def plot_curve(train_losses, val_losses, path):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title("Training Curve (Image + I + EMA)")
    plt.legend()
    plt.tight_layout()
...
def plot_scatter(y_true, y_pred, path):
    ...
    plt.title("Predicted vs True (Image + I + EMA)")
    plt.tight_layout()
    plt.savefig(path, dpi=300)
    plt.close()
...
best_model_path = os.path.join(model_dir, "best_resnet18_with_i_ema.pth")
final_model_path = os.path.join(model_dir, "final_resnet18_with_i_ema.pth")
...
for epoch in range(1, EPOCHS + 1):
    ...
    # 用 EMA 模型做验证
    val_loss, _, _, _, _, _ = evaluate(ema_model, val_loader, criterion, device)
...
plot_curve(train_losses, val_losses, os.path.join(fig_dir, "training_curve_with_i_ema.png"))
...
# 测试集评估
# ======================
print("\nLoading best EMA model for test evaluation...")
ema_model.load_state_dict(torch.load(best_model_path, map_location=device))

test_loss, test_rmse, test_mae, test_r2, y_pred, y_true = evaluate(ema_model, test_loader, criterion, device)
...
plot_scatter(y_true, y_pred, os.path.join(fig_dir, "test_scatter_with_i_ema.png"))

print("GPU training confirmed:", torch.cuda.is_available())
print("Final device:", next(ema_model.parameters()).device)