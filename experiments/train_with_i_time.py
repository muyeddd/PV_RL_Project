...
def evaluate(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for images, L, I, time_feat in loader:
            ...
            if torch.cuda.is_available():
                with torch.amp.autocast("cuda"):
                    outputs = model(images, I, time_feat)
...
def plot_curve(train_losses, val_losses, path):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title("Training Curve (Image + I + Time)")
    plt.legend()
    plt.tight_layout()
...
def plot_scatter(y_true, y_pred, path):
    ...
    plt.title("Predicted vs True (Image + I + Time)")
    plt.tight_layout()
    plt.savefig(path, dpi=300)
    plt.close()
...
best_model_path = os.path.join(model_dir, "best_resnet18_with_i_time.pth")
final_model_path = os.path.join(model_dir, "final_resnet18_with_i_time.pth")
...
for epoch in range(1, EPOCHS + 1):
    ...
    for images, L, I, time_feat in train_loader:
        images = images.to(device)
        L = L.to(device).unsqueeze(1)
        I = I.to(device)
        time_feat = time_feat.to(device)
    ...
    total_val = 0.0
    with torch.no_grad():
        for images, L, I, time_feat in val_loader:
            images = images.to(device)
            L = L.to(device).unsqueeze(1)
            I = I.to(device)
            time_feat = time_feat.to(device)
...
plot_curve(train_losses, val_losses, os.path.join(fig_dir, "training_curve_with_i_time.png"))
...
plot_scatter(y_true, y_pred, os.path.join(fig_dir, "test_scatter_with_i_time.png"))
