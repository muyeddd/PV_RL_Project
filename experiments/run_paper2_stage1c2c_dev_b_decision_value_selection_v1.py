...
OUT = BASE / "p2_1c_2c_dev_b_decision_value_selection_v1"
STAGE = "P2-1C-2C-DEV-B-v1"
...
K_COLUMNS = ["k", "k_selection_eligible", "Score_percent", "selected"]
    ...
    for policy, k in [("True-State", None), ("Point", None)] + [("UA", k) for k in ks]:
        for budget in budgets:
            ...
            for year, asset in assets.items():
                ...
                try:
                    result = dev_a.generic_rollout(predecessor, guarded, emulator, asset, policy, k,
                                                  threshold, len(ids), ids, audit)
                except predecessor.UnsupportedPerceptionQuery as exc:
                    exc.context.update(policy=policy, k=k, budget=budget, threshold=threshold)
                    raise  # No retry, partial metrics, or threshold change.
...
def analyze_selection(results, ids, thresholds, counts, point_pass, eligible):
...
def protocol(mode):
    manifest = {"stage": STAGE, "title": TITLE, "mode": mode,
                ...
                "support": "unchanged frozen sampler; no fallback or retry; unsupported evaluation fails closed"}
    if mode == "dev-b":
...
def main():
    ...
    try:
        ...
        results, uses = evaluate_frozen(dev_a, predecessor, frozen, emulator, assets, thresholds,
        ...
        gates[GATES[8]] = all(use["threshold_hex"] == float(thresholds[use["policy"], use["k"], use["budget"]]).hex()
                              for use in uses)
        # Fairness is completed before trajectory_metrics/analyze_selection.
        fair_rows, counts, point_pass, eligible = fairness(results, budgets, ks)
        gates[GATES[10]] = True
        if args.mode == "smoke":
            ...
            cells, trajectories, k_rows, references, selection = analyze_selection(
                results, ids, thresholds, counts, point_pass, eligible)
        ...
        gates[GATES[11]] = True
        gates[GATES[12]] = (len(uses) == len(results) == (2 + len(ks)) * len(budgets) * 2
                            and gates[GATES[8]])
    ...
    if args.mode == "smoke":
        ...
        complete = (len(fair_rows) == 48 and len(cells) == 48 and len(trajectories) == 9600
                    and len(k_rows) == 6 and len(references) == 36)
    try:
