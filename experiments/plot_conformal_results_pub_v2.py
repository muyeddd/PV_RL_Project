...
# =========================
def plot_tradeoff(summary_df, ax=None):
    own_fig = ax is None
    if own_fig:
    ...
    ax1.plot(conf, picp, marker="o", markersize=5.3,
             color=C_BLUE, label="Empirical coverage")
    ax1.plot(conf, target, linestyle="--",
             color=C_RED, label="Target coverage")
    ...
    ax2 = ax1.twinx()
    ax2.plot(conf, mpiw, marker="s", markersize=5.0,
             color=C_ORANGE, label="Mean interval width")
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig1_tradeoff_final")


# =========================
# 图2：90% Prediction Interval Scatter
# =========================
def plot_interval_scatter(test_df, sample_n=150, random_state=42, ax=None):
    own_fig = ax is None
    if own_fig:
    ...
    ax.errorbar(
        ...
        markersize=2.4,
        color=C_LIGHT_BLUE,
        ecolor=(76/255, 120/255, 168/255, 0.16),
        elinewidth=0.55,
        capsize=0.8,
        alpha=0.82,
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig2_interval_scatter_90_final")
...
def plot_coverage_by_lbin(test_df, ax=None):
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig3_coverage_by_lbin_final")
...
def plot_coverage_by_ibin(test_df, ax=None):
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig4_coverage_by_ibin_final")
...
def plot_multipanel(summary_df, test_df):
    ...
    plot_tradeoff(summary_df, ax=axes[0, 0])
    plot_interval_scatter(test_df, sample_n=150, ax=axes[0, 1])
    ...
    png_path = os.path.join(fig_dir, "fig7_multipanel_conformal_final.png")
    pdf_path = os.path.join(fig_dir, "fig7_multipanel_conformal_final.pdf")
...
if __name__ == "__main__":
    ...
    plot_interval_scatter(test_df, sample_n=150)
