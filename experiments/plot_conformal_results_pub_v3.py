...
fig_dir = os.path.join(result_dir, "figures_final_v3")
...
# ============================================================
# 全局论文风格：IEEE / Elsevier / Energy / Applied Energy
# ============================================================
plt.rcParams.update({
...
# 通用分箱覆盖率函数
# 注意：
# bin_values 用来决定样本属于哪个 bin
# y_true / lower / upper 用来判断 L 是否被预测区间覆盖
# ============================================================
...
def plot_tradeoff(summary_df, ax=None, show_legend=True):
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig1_tradeoff_final_v3")
...
def plot_interval_scatter(test_df, sample_n=120, random_state=42, ax=None):
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig2_interval_scatter_90_final_v3")


# ============================================================
# 图3：Coverage by Power Loss Bin
# 红色 + hatch 表示 under-coverage
# ============================================================
def plot_coverage_by_lbin(test_df, ax=None, annotate_target=True):
    own_fig = ax is None
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig3_coverage_by_lbin_final_v3")


# ============================================================
# 图4：Coverage by Irradiance Bin
# 红色 + hatch 表示 under-coverage
# ============================================================
def plot_coverage_by_ibin(test_df, ax=None, annotate_target=True):
    own_fig = ax is None
    ...
    if own_fig:
        fig.tight_layout()
        save_fig(fig, "fig4_coverage_by_ibin_final_v3")
...
def plot_multipanel(summary_df, test_df):
    ...
    plot_coverage_by_lbin(test_df, ax=axes[1, 0], annotate_target=True)
    plot_coverage_by_ibin(test_df, ax=axes[1, 1], annotate_target=True)
    ...
    png_path = os.path.join(fig_dir, "fig7_multipanel_conformal_final_v3.png")
    pdf_path = os.path.join(fig_dir, "fig7_multipanel_conformal_final_v3.pdf")
    svg_path = os.path.join(fig_dir, "fig7_multipanel_conformal_final_v3.svg")
...
if __name__ == "__main__":
    ...
    print("\nFinal v3 publication-style figures saved to:")
    print(fig_dir)