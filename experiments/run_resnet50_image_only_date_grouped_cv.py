"""Run leakage-free formal ResNet50 Image-only cross-validation for date_grouped_v1."""
...
FORMAL_OUTPUT_ROOT = (
    PROJECT_ROOT / "outputs" / "date_grouped_v1" / "resnet50_image_only" / "formal_cv"
)
FOLDS = (1, 2, 3, 4)
SEED = 42
EPOCHS = 50
...
def validate_output_root_isolation() -> None:
    ...
    if actual != expected or "resnet50_with_i" in {
        part.lower() for part in actual.parts
    }:
        raise RuntimeError(
            "Image-only formal output root is not isolated from ResNet50+I: "
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run formal date_grouped_v1 ResNet50 Image-only cross-validation"
    )
...
def preflight_formal_fold(fold: int) -> dict[str, Any]:
    ...
    pretrained = training._pretrained_provenance(True)
    if not pretrained["pretrained_cache_present_before_init"]:
        raise FileNotFoundError(
            "ImageNet ResNet50 weights are absent from the torchvision cache"
        )
    return audit
...
def validate_fold_outputs(fold: int, output_dir: Path) -> dict[str, Any]:
    ...
    if not metadata["pretrained"]["pretrained_load_success"]:
        raise RuntimeError(f"Fold {fold} did not load ImageNet pretrained weights")
...
def write_cv_summary(started_at_utc: str) -> dict[str, Any]:
    ...
    summary = {
        "schema_version": 1,
        "training_version": "resnet50_image_only_date_grouped_v1",
