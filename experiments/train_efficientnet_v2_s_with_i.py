...
# 模型
# ======================
model = SolarEfficientNetV2SWithI(dropout=0.3).to(device)
...
def plot_curve(train_losses, val_losses, path):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title("Training Curve (EfficientNetV2-S + I)")
    plt.legend()
    plt.tight_layout()
...
def plot_scatter(y_true, y_pred, path):
    ...
    plt.title("Predicted vs True (EfficientNetV2-S + I)")
    plt.tight_layout()
    plt.savefig(path, dpi=300)
    plt.close()
...
best_model_path = os.path.join(model_dir, "best_efficientnet_v2_s_with_i.pth")
final_model_path = os.path.join(model_dir, "final_efficientnet_v2_s_with_i.pth")
...
plot_curve(train_losses, val_losses, os.path.join(fig_dir, "training_curve_efficientnet_v2_s_with_i.png"))
...
plot_scatter(y_true, y_pred, os.path.join(fig_dir, "test_scatter_efficientnet_v2_s_with_i.png"))
