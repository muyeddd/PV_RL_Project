...
train_files, val_files, test_files = split_files(files, seed=SPLIT_SEED)
...
# 预处理
# ======================
# ConvNeXt-Tiny 官方预训练权重对应的预处理建议是 crop_size=224, resize_size=236
# 这里我们用最稳妥的 224 训练/验证
...
def evaluate(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for images, L, I, _time_feat in loader:
...
def plot_curve(train_losses, val_losses, path):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title("Training Curve (ConvNeXt-Tiny + I)")
    plt.legend()
    plt.tight_layout()
...
def plot_scatter(y_true, y_pred, path):
    ...
    plt.title("Predicted vs True (ConvNeXt-Tiny + I)")
    plt.tight_layout()
    plt.savefig(path, dpi=300)
    plt.close()
...
best_model_path = os.path.join(model_dir, f"best_convnext_tiny_with_i_seed{RUN_SEED}.pth")
final_model_path = os.path.join(model_dir, f"final_convnext_tiny_with_i_seed{RUN_SEED}.pth")
...
for epoch in range(1, EPOCHS + 1):
    ...
    for images, L, I, _time_feat in train_loader:
        images = images.to(device)
        L = L.to(device).unsqueeze(1)
        I = I.to(device)
    ...
    total_val = 0.0
    with torch.no_grad():
        for images, L, I, _time_feat in val_loader:
            images = images.to(device)
            L = L.to(device).unsqueeze(1)
            I = I.to(device)
...
plot_curve(train_losses, val_losses, os.path.join(fig_dir, f"training_curve_convnext_tiny_with_i_seed{RUN_SEED}.png"))
...
plot_scatter(y_true, y_pred, os.path.join(fig_dir, f"test_scatter_convnext_tiny_with_i_seed{RUN_SEED}.png"))
