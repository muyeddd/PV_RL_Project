#!/usr/bin/env python
"""P2-1E-1-v1.1: Gym/POMDP Environment Contract and Support-Admissible Regression Audit.
...
STAGE = "P2-1E-1-v1.1"
TITLE = "Gym/POMDP Environment Contract and Support-Admissible Regression Audit"
SELF = "experiments/run_paper2_stage1e1_gym_pomdp_environment_audit_v1_1.py"
ENV_FILE = "experiments/paper2_gym_pomdp_env_v1.py"
V1_AUDIT_FILE = "experiments/run_paper2_stage1e1_gym_pomdp_environment_audit_v1.py"
EXPECTED_CORE_ENV_BLOB = "5233c7d45cb24db3fd55c56ba658b4ae0b9cafb4"
EXPECTED_V1_AUDIT_BLOB = "4db745164fc8a6a0aa9d5b17fcd6eb8e5214bce0"
SUPPORT_DIRECTORY = core.BASE / "p2_1c2a_clean_mechanics_operational_support_audit_v1"
SUPPORT_ARTIFACT_HASHES = {
    "audit_summary.json": "4e8f7a147f0288fc395c348f60c8ea9850f3d223823f27863e14c47a9f2c1249",
    "primary_eta_support_table.csv": "a3ede46b722fd5e4c325793549cca1a5a5fcbcdeb0dee5f7c47b64114c9c72bf",
}
SUPPORT_ATOL = 1e-15
OUTPUT = core.BASE / "p2_1e_1_gym_pomdp_environment_audit_v1_1"
ENV_ROOTS = {"YEAR1": 1020001, "YEAR2": 1030001}
POPULATION_SIZE = 300
PERCEPTION_SEED = 20260906
TRUE_STATE_REGRESSION_POLICIES = ("NEVER_CLEAN", "DAILY_CLEAN", "TRUE_THRESHOLD_0.05")
PERCEPTION_ADMISSIBLE_REGRESSION_POLICIES = ("TRUE_THRESHOLD_0.05",)
...
GATES = dict(enumerate([
    ...
    "core_environment_historical_blob_exact", "frozen_support_reference_exact",
], start=1))
...
def provenance(audit, manifest):
    ...
    historical = historical_environment_provenance(head)
    gate(audit, 28, all(record["exact"] for record in historical.values()), historical)
    manifest.update(historical)
    manifest["core_environment_unchanged"] = historical["core_environment_provenance"]["exact"]
    manifest["v1_failure_evidence_script_unchanged"] = historical["v1_failure_audit_provenance"]["exact"]
    support = frozen_support_reference()
    gate(audit, 29, support["verified"], support)
    manifest["support_reference"] = support
...
def historical_environment_provenance(head):
    records = {}
    for key, path, expected in (
        ("core_environment_provenance", ENV_FILE, EXPECTED_CORE_ENV_BLOB),
        ("v1_failure_audit_provenance", V1_AUDIT_FILE, EXPECTED_V1_AUDIT_BLOB),
    ):
        # script_record reads HEAD:path using rev-parse and also verifies that
        # the working file is unchanged. Compare that tree blob to the historical
        # pin here; its optional expected argument would additionally query the
        # earlier reward checkpoint, which predates these environment files.
        record = core.script_record(path, head)
        actual = record["git_blob"]
        check(actual == expected, f"G28 historical HEAD tree blob mismatch: {path}")
        records[key] = {"path": path, "expected_git_blob": expected, "actual_git_blob": actual,
                        "git_tree_source": f"{head}:{path}", "working_tree_sha256": record["sha256"],
                        "working_tree_matches_HEAD": True, "exact": actual == expected}
    return records


def frozen_support_reference():
    payloads, artifacts = {}, {}
    for name, expected in SUPPORT_ARTIFACT_HASHES.items():
        path = SUPPORT_DIRECTORY / name
        check(path.is_file(), f"G29 missing frozen support artifact: {path}")
        payload = path.read_bytes()
        actual = core.sha(payload)
        check(actual == expected, f"G29 frozen support SHA256 mismatch: {name}")
        payloads[name] = payload
        artifacts[name] = {"path": str(path), "sha256": actual, "expected_sha256": expected, "verified": True}
    prior = json.loads(payloads["audit_summary.json"])
    check(prior.get("stage") == "P2-1C-2A" and prior.get("stage_pass") is True
          and ("failed_gates" not in prior or prior["failed_gates"] == []),
          "G29 frozen support audit must be successful, never partial or failed")
    table = read_csv(payloads["primary_eta_support_table.csv"])
    required = {"eta", "threshold", "YEAR1_margin_exceed_fraction", "YEAR2_margin_exceed_fraction",
                "support_safe_both_years"}
    check(required <= set(table.columns), "G29 frozen combined-year support schema")
    # Query by scientific identity, never by row position or support/performance ranking.
    selected = table.loc[table.eta.eq(0.95) & table.threshold.eq(0.05)]
    check(len(selected) == 1, "G29 exactly one eta=.95 / threshold=.05 combined-year row required")
    raw = selected.to_dict(orient="records")[0]
    safe = raw["support_safe_both_years"]
    check(isinstance(safe, (bool, np.bool_)) and bool(safe), "G29 frozen support_safe_both_years must be boolean True")
    for year in ("YEAR1", "YEAR2"):
        value = raw[f"{year}_margin_exceed_fraction"]
        check(np.isfinite(value) and value == 0.0, f"G29 {year} frozen margin exceed fraction must equal zero")
    reference = prior["support_reference"]
    expected_bounds = {"historical_valid_state_max": 0.160039312862104,
                       "perception_margin_max": 0.170039312862104}
    bound_sources = {}
    for field, expected in expected_bounds.items():
        ...
        check(isinstance(actual, (int, float)) and not isinstance(actual, bool)
              and np.isfinite(actual) and np.isclose(actual, expected, atol=SUPPORT_ATOL, rtol=0),
              f"G29 frozen support reference mismatch: {field}")
        bound_sources[field] = {"path": str(SUPPORT_DIRECTORY / "audit_summary.json"),
                                "field": f"support_reference.{field}", "value": actual}
    return {"stage": prior["stage"], "verified": True,
            "audit_summary": artifacts["audit_summary.json"],
            "primary_eta_support_table": artifacts["primary_eta_support_table.csv"],
            "audit_success_fields": {"stage_pass": prior["stage_pass"], "failed_gates_present": "failed_gates" in prior,
                                     "failed_gates": prior.get("failed_gates")},
            "query": {"eta": raw["eta"], "threshold": raw["threshold"]},
            "schema": "combined-year row selected by eta and threshold", "selected_row_original_fields": raw,
            "YEAR1": {key: value for key, value in raw.items() if key.startswith("YEAR1_")},
            "YEAR2": {key: value for key, value in raw.items() if key.startswith("YEAR2_")},
            "support_safe_both_years": bool(safe),
            "support_safe_source": {"path": str(SUPPORT_DIRECTORY / "primary_eta_support_table.csv"),
                                    "field": "support_safe_both_years", "value": bool(safe),
                                    "scope": "YEAR1 AND YEAR2 combined frozen field"},
            "historical_valid_state_max": reference["historical_valid_state_max"],
            "perception_margin_max": reference["perception_margin_max"], "bound_sources": bound_sources,
            "numerical_tolerance": {"atol": SUPPORT_ATOL, "rtol": 0, "rounding": False},
            "interpretation": "Prior operational support admissibility of TRUE_THRESHOLD_0.05 only; runtime sampler support checks remain mandatory",
            "not_established": ["support for every L in [0,1]", "NEVER_CLEAN full-episode perception support",
                                "DAILY_CLEAN perception support safety", "absence of unsupported queries in future PPO"]}
...
def frozen_population(assets):
    ...
    expected = {(year, tid, policy, day) for year in ENV_ROOTS for tid in range(10) for policy in TRUE_STATE_REGRESSION_POLICIES for day in range(365)}
...
def audit_episodes(assets, n, audit):
    ...
    for year in ENV_ROOTS:
        for tid in range(n):
            ...
            for policy in TRUE_STATE_REGRESSION_POLICIES:
                reference = frozen.loc[frozen.year.eq(year) & frozen.trajectory_id.eq(tid) & frozen.policy.eq(policy)].sort_values("day_index")
                results = {}
                shared_actions = None
                # True-State runs every frozen probe. Point/UA run only the
                # pre-validated support-admissible trajectory, after its True-State
                # rollout has supplied the complete action sequence.
                active_modes = (("True-State", "Point", "UA")
                                if policy in PERCEPTION_ADMISSIBLE_REGRESSION_POLICIES
                                else ("True-State",))
                for mode in active_modes:
                    if mode != "True-State":
                        check(shared_actions is not None and len(shared_actions) == 365,
                              "Point/UA require the corresponding True-State action sequence")
                    with ExitStack() as guard:
                        ...
                        env.close()
                if policy not in PERCEPTION_ADMISSIBLE_REGRESSION_POLICIES:
                    # Not a perception success or support claim: only True-State
                    # was requested for this physical/reward regression probe.
                    continue
    ...
    expected_keys = {
        (year, tid, mode, policy, seed)
        for year in ENV_ROOTS for tid in range(n) for seed in (1, 999)
        for policy in TRUE_STATE_REGRESSION_POLICIES
        for mode in (("True-State", "Point", "UA")
                     if policy in PERCEPTION_ADMISSIBLE_REGRESSION_POLICIES else ("True-State",))
    }
    actual_keys = set(summaries[["year", "trajectory_id", "observation_mode", "policy", "gym_seed"]]
                      .itertuples(index=False, name=None))
    expected_pairs = {(year, tid, policy) for year in ENV_ROOTS for tid in range(n)
                      for policy in PERCEPTION_ADMISSIBLE_REGRESSION_POLICIES}
    check(len(summaries) == len(expected_keys) == 2 * n * (3 + 2) * 2
          and actual_keys == expected_keys
          and len(pairs) == len(expected_pairs) == 2 * n
          and {(row["year"], row["trajectory_id"], row["policy"]) for row in pairs} == expected_pairs,
          "Exact role-separated audit episode and pairing populations")
    ...
    gate(audit, 18, summaries.regression_pass.all(), {
                                                  "True_State_policies": list(TRUE_STATE_REGRESSION_POLICIES),
                                                  "Point_UA_policies": list(PERCEPTION_ADMISSIBLE_REGRESSION_POLICIES),
                                                  "daily_atol": ATOL, "rtol": RTOL,
    ...
    gate(audit, 21, all(row["q50_season_bitwise_equal"] for row in pairs),
         {"paired_episodes": len(pairs), "policies": list(PERCEPTION_ADMISSIBLE_REGRESSION_POLICIES),
          "domain": "pre-validated support-admissible threshold trajectory", "all_differences": 0})
    ...
    gate(audit, 24, True, {"policies": list(PERCEPTION_ADMISSIBLE_REGRESSION_POLICIES),
                          "comparison": "True-State/Point/UA: identical externally supplied threshold actions; exact hidden dynamics/reward and indices",
                          "NEVER_CLEAN_Point_UA_full_episode_tested": False})
...
def protocol(mode):
    return {"stage": STAGE, "title": TITLE, "mode": mode, "expected_versions": EXPECTED_VERSIONS,
            ...
            "observation_modes": list(MODES), "trace_policies": list(TRUE_STATE_REGRESSION_POLICIES), "gym_reset_test_seeds": [1, 999],
            ...
            "audit_protocol_revision": "v1.1 support-domain correction after v1 fail-closed diagnostic",
            "v1_failure_class": "KNOWN_EXTRAPOLATIVE_TRAJECTORY_REQUESTED_PERCEPTION",
            "v1_failure": "AUDIT_PROTOCOL_REQUIRED_PERCEPTION_ON_KNOWN_EXTRAPOLATIVE_NEVER_CLEAN_TRAJECTORY",
            "true_state_regression_policies": list(TRUE_STATE_REGRESSION_POLICIES),
            "perception_admissible_regression_policies": list(PERCEPTION_ADMISSIBLE_REGRESSION_POLICIES),
            "provenance_hardening": "P2-1E-1-v1.1-R1",
            "support_reference": None,  # Populated only by successful G29 verification.
            "NEVER_CLEAN_role": "TRUE_STATE_REWARD_PHYSICS_REGRESSION_AND_EXTRAPOLATION_STRESS_ONLY",
            "NEVER_CLEAN_perception_role": "NOT_FULL_EPISODE_ELIGIBLE; extrapolation diagnostic",
            "DAILY_CLEAN_perception_role": "NOT_REQUESTED_BY_THIS_AUDIT; no full-episode support claim",
            "support_contract_unchanged": True, "core_environment_unchanged": None,
            "v1_failure_evidence_script_unchanged": None,  # Populated by G28.
            "core_environment_change": False, "frozen_perception_change": False,
            "reward_change": False, "natural_dynamics_change": False,
            "frozen_perception_support": {"local_radius_abs_L": 0.01, "local_min_samples": 20,
                                          "local_min_dates": 3, "fallback": False,
                                          "role": "DOCUMENTATION_ONLY; no sampler parameter override"},
            "known_v1_natural_support_diagnostic": {
                "role": "DIAGNOSTIC_ONLY", "formal_hard_gate": False,
                "year": "YEAR1", "trajectory_id": 0, "policy": "NEVER_CLEAN",
                "day_index": 133, "date": "2021-12-20", "L_true": 0.21687332636557485,
                "local_rows": 19, "local_dates": 6, "candidate_blocks": 17},
            "episode_population": {"True_State": 2 * (10 if mode == "formal" else 2) * 3 * 2,
                                   "Point_UA": 2 * (10 if mode == "formal" else 2) * 2 * 2,
                                   "Point_UA_pairings": 2 * (10 if mode == "formal" else 2)},
            "checkers": "All three modes; actual unsupported queries remain hard failures",
            "selection_basis": "Frozen P2-1C support contract only; no reward/performance comparison or policy selection"}
...
def publish(out, manifest, api, summaries, pairs, trace, checkers, audit, assets):
    ...
    historical = historical_environment_provenance(manifest["current_HEAD"])
    check(all(record == manifest[key] for key, record in historical.items()),
          "G28 historical environment/v1 evidence changed during audit")
    check(frozen_support_reference() == manifest["support_reference"],
          "G29 frozen support evidence changed during audit")
...
def main():
    ...
    try:
        ...
        api = {"stage": STAGE, "mode": args.mode,
               ...
               "audit_domain": {"True_State": list(TRUE_STATE_REGRESSION_POLICIES),
                                "Point_UA": list(PERCEPTION_ADMISSIBLE_REGRESSION_POLICIES),
                                "NEVER_CLEAN_Point_UA_full_episode_supported": "NOT_CLAIMED"},
