...
train_ds = SolarDataset(img_dir=img_dir, transform=transform_train)
train_ds.files = train_files

val_ds = SolarDataset(img_dir=img_dir, transform=transform_eval)
val_ds.files = val_files

test_ds = SolarDataset(img_dir=img_dir, transform=transform_eval)
...
# Mixup
# ======================
def mixup_batch(images, targets, i_values, alpha=0.4):
    if alpha <= 0:
        return images, targets, i_values, targets, 1.0
    ...
    mixed_i = lam * i_values + (1 - lam) * i_values[index]

    return mixed_images, mixed_targets, mixed_i, targets[index], lam
...
def plot_curve(train_losses, val_losses, path):
    ...
    plt.title("Training Curve (ResNet18 + I + 192 + Huber + Mixup)")
    plt.legend()
    plt.tight_layout()
...
def plot_scatter(y_true, y_pred, path):
    ...
    plt.title("Predicted vs True (ResNet18 + I + 192 + Huber + Mixup)")
    plt.tight_layout()
    plt.savefig(path, dpi=300)
    plt.close()
...
best_model_path = os.path.join(model_dir, "best_resnet18_with_i_192_huber_mixup.pth")
final_model_path = os.path.join(model_dir, "final_resnet18_with_i_192_huber_mixup.pth")
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch in train_loader:
        ...
        # Mixup
        images_mix, L_mix, I_mix, _, _ = mixup_batch(images, L, I, alpha=MIXUP_ALPHA)

        optimizer.zero_grad(set_to_none=True)

        if torch.cuda.is_available():
            with torch.amp.autocast("cuda"):
                outputs = model(images_mix, I_mix)
...
plot_curve(train_losses, val_losses, os.path.join(fig_dir, "training_curve_resnet18_i_192_huber_mixup.png"))
...
plot_scatter(y_true, y_pred, os.path.join(fig_dir, "test_scatter_resnet18_i_192_huber_mixup.png"))
