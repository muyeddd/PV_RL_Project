...
"""
Paper2 / P2-1C-2B
Online Counterfactual Perception Integration Audit v1.1
...
v1.1 IMPLEMENTATION CORRECTION (pre-RL, no scientific retuning)
---------------------------------------------------------------
P2-1C-2B-R1 proved row-for-row that the frozen P2-0C-3C semantics are:
  * preserve original Paper1 CSV row order;
  * rng.choice(sorted(BLOCK10 blocks));
  * rng.choice(rows, p=Gaussian L-distance weights);
  * source_lower_clipped=True implies generated lower=0;
  * source_lower_clipped=False may ALSO generate lower=0 after transported
    lower is clipped with np.clip(...,0,1).
This v1.1 applies only that verified implementation correction. The original
support rules, seeds, scientific hyperparameters and regression tolerances
are unchanged.
...
# R1-verified frozen provenance. A mismatch is a hard stop, not an invitation
# to regenerate/tune assets.
EXPECTED_HASHES = {
    "paper1_cqr": "9136bf89b694517924d47a9e8622d7201e57b1dc719bff508a0d507703adc192",
    "wapp_power_bridge": "08b3f1cfc4be4cc5955b0078481c3f17ea0133dff66c0ef36c829f0009d85673",
    "frozen_emulator_script": "73da2bb4983be433e27ec3750cde87be764e33900f85ca258843dd7a8e564738",
    "frozen_trajectory_bank": "cc1c0c98abd8233035d8913d9589fc5a3497d6c9d4de4f633a278cac4362963f",
    "frozen_seed_metrics": "c5c877816486b55624a23454268551832dcbfdb21f61fa1268c8e8494339c426",
}
...
def load_paper1_cqr(path: Path) -> pd.DataFrame:
    """
    Load Paper1 DEV in ORIGINAL CSV row order.

    R1 proved that source-row order is part of the frozen stochastic sampler
    implementation because rng.choice is applied to block-local row indices.
    """
    df = pd.read_csv(path, encoding="utf-8-sig")
    required = {
        "sample_id", "date", "timestamp", "role", "true_L",
        "q50", "lower", "upper", "width", "covered",
        "lower_clipped", "upper_clipped",
    }
    missing = required.difference(df.columns)
    if missing:
    ...
    if not np.array_equal(
        ...
        raise RuntimeError("Paper1 timestamp/date mismatch.")
    ...
    df["upper_clipped"] = parse_bool_series(
        df["upper_clipped"], "upper_clipped"
    )
    df["covered"] = parse_bool_series(df["covered"], "covered")

    if not (
        (df["lower"] <= df["q50"])
        & (df["q50"] <= df["upper"])
    ).all():
        raise RuntimeError("Paper1 interval ordering violated.")
    if not np.allclose(
        ...
        raise RuntimeError("Paper1 width identity drift.")

    lower_zero = np.isclose(
        df["lower"].to_numpy(dtype=float),
        0.0,
        atol=1e-12,
        rtol=0.0,
    ...
    if not np.array_equal(
        ...
        raise RuntimeError("Paper1 lower_clipped semantic drift.")
    if df["upper_clipped"].astype(bool).any():
        raise RuntimeError("Unexpected Paper1 upper clipping.")

    # EXACT frozen 0C-3C block ID construction.
    df["source_block_id"] = (
        df["date"]
        + "|"
        + df["timestamp"].dt.floor(
            f"{BLOCK_MINUTES}min"
        ).astype(str)
    )

    # NO sorting here. Preserve original CSV order.
...
def load_wapp_power_bridge(path: Path) -> pd.DataFrame:
    df = pd.read_csv(path, encoding="utf-8-sig")
    required = {
        "date", "L_power_proxy", "bridge_valid",
        "power_bridge_model",
    }
    missing = required.difference(df.columns)
    if missing:
        raise RuntimeError(
            f"WAPP bridge missing columns: {sorted(missing)}"
        )

    df["bridge_valid"] = parse_bool_series(
        df["bridge_valid"], "bridge_valid"
    )
    df = df.loc[df["bridge_valid"]].copy().reset_index(drop=True)

    if len(df) != EXPECTED_VALID_STATES:
        raise RuntimeError(
            f"Expected {EXPECTED_VALID_STATES} valid WAPP bridge days, "
            f"found {len(df)}"
        )
    if set(df["power_bridge_model"].astype(str)) != {
        "COMMON_TEMPERATURE_PVWATTS_RATIO"
    }:
        raise RuntimeError("Unexpected WAPP bridge model.")

    df["date"] = pd.to_datetime(
        df["date"], errors="raise"
    ).dt.strftime("%Y-%m-%d")
    df["L_power_proxy"] = pd.to_numeric(
        df["L_power_proxy"], errors="raise"
    )
    if df["date"].duplicated().any():
        raise RuntimeError("Duplicate WAPP bridge dates.")
    if not np.isfinite(
        df["L_power_proxy"].to_numpy(dtype=float)
    ).all():
        raise RuntimeError("Non-finite WAPP bridge L values.")

    return df


def load_frozen_trajectory_bank(path: Path) -> pd.DataFrame:
    df = pd.read_csv(path, encoding="utf-8-sig")
    required = {
        "date", "L_true", "q50", "lower", "upper", "width",
        "lower_clipped", "perception_seed", "source_sample_id",
        "source_date", "source_block10_id", "source_true_L",
        "source_lower_clipped", "candidate_samples",
        "candidate_dates", "candidate_blocks",
    }
    missing = required.difference(df.columns)
    if missing:
        raise RuntimeError(
            f"Frozen trajectory bank missing: {sorted(missing)}"
        )

    expected = EXPECTED_VALID_STATES * len(PERCEPTION_SEEDS)
    if len(df) != expected:
        raise RuntimeError(
            f"Frozen trajectory-bank rows drift: {len(df)} != {expected}"
        )
    ...
    df["perception_seed"] = df["perception_seed"].astype(int)
    df["lower_clipped"] = parse_bool_series(
        df["lower_clipped"], "bank_lower_clipped"
    )
    df["source_lower_clipped"] = parse_bool_series(
        df["source_lower_clipped"], "bank_source_lower_clipped"
    )

    seeds = tuple(
        sorted(df["perception_seed"].unique().tolist())
    )
    if seeds != tuple(sorted(PERCEPTION_SEEDS)):
        raise RuntimeError("Frozen trajectory-bank seed drift.")
...
class OnlineBlock10Emulator:
    """
    Online implementation of the R1-verified frozen BLOCK10 sampler semantics.

    Candidate retrieval for sampling preserves original Paper1 CSV row order.
    Sorted views are used ONLY for vectorized support-count diagnostics.
    """
    ...
    def __init__(self, source: pd.DataFrame):
        ...
        self.date = self.source["date"].astype(
            str
        ).to_numpy(dtype=object)
        self.block = self.source["source_block_id"].astype(
            str
        ).to_numpy(dtype=object)
        self.sample_id = self.source["sample_id"].to_numpy(dtype=object)
        ...
        # Sorted view for fast support/candidate lookup; original CSV row
        # order is restored before frozen rng.choice is applied.
    ...
    def _candidate_indices(self, query_L: float) -> np.ndarray:
        # Fast range lookup, then restore EXACT original Paper1 CSV row order.
        q = float(query_L)
        lo = np.searchsorted(
    ...
    def support_summary(self, queries) -> dict:
        ...
        for d in self.date_levels:
            ...
            dlo = np.searchsorted(
                arr, q - LOCAL_RADIUS, side="left"
            )
            dhi = np.searchsorted(
                arr, q + LOCAL_RADIUS, side="right"
            )
    ...
    def sample_one(
        ...
        cand = self._candidate_indices(query_L)

        if len(cand) < MIN_LOCAL_SAMPLES:
            raise RuntimeError(
                f"Unsupported query {query_L:.9f}: "
                f"{len(cand)} local rows."
            )
        ...
        if len(dates) < MIN_LOCAL_DATES:
            raise RuntimeError(
                f"Unsupported query {query_L:.9f}: "
                f"{len(dates)} local dates."
            )

        blocks = sorted(
            np.unique(self.block[cand]).tolist()
        )
        if len(blocks) == 0:
            raise RuntimeError("No eligible BLOCK10 source block.")

        # EXACT frozen 0C-3C stochastic API.
        chosen_block = str(rng.choice(blocks))
        rows = cand[self.block[cand] == chosen_block]

        distances = np.abs(self.L[rows] - query_L)
        weights = np.exp(
            -0.5
            * np.square(
                distances / KERNEL_BANDWIDTH
            )
        )
        ...
        src = int(rng.choice(rows, p=weights))

        q50 = float(
            np.clip(
                query_L + self.dq50[src],
                0.0,
                1.0,
            )
        )
        ...
        raw_lower = float(
            query_L + self.dlower[src]
        )

        if self.lower_clipped[src]:
            lower = 0.0
        else:
            # R1-verified frozen semantic:
            # a non-source-clipped interval may become clipped after transport.
            lower = float(
                np.clip(raw_lower, 0.0, 1.0)
            )
        ...
        generated_lower_clipped = bool(
            np.isclose(
                lower, 0.0, atol=1e-12, rtol=0.0
            )
        )
        source_lower_clipped = bool(
            self.lower_clipped[src]
        )
        ...
        # Only the implication is frozen:
        # source clipped => generated clipped.
        if source_lower_clipped and not generated_lower_clipped:
            raise RuntimeError(
                "Frozen source lower clipping was not preserved."
            )
        ...
        return {
            ...
            "source_index": src,
            "source_sample_id": self.sample_id[src],
            ...
            "new_transport_lower_clip": new_transport_lower_clip,
...
def perception_metrics(frame: pd.DataFrame) -> dict:
    ...
    implication_pass = bool(
        generated_clip[source_clip].all()
    )
    expected_new_clip = (
        (~source_clip)
        & (raw_lower <= 1e-12)
    )
    observed_new_clip = frame[
        "new_transport_lower_clip"
    ].astype(bool).to_numpy()

    boundary_semantics_pass = bool(
        np.array_equal(
            expected_new_clip,
            observed_new_clip,
        )
    ...
    return {
        ...
        "source_lower_clipped_fraction": float(
            source_clip.mean()
        ),
        "new_transport_lower_clip_fraction": float(
            observed_new_clip.mean()
        ),
        "rho_width_abs_error": spearman_safe(
            width, np.abs(err)
        ),
        "finite_pass": bool(
            np.isfinite(
        ...
        "ordering_pass": bool(
            (lower >= -1e-12).all()
            and (lower <= q50 + 1e-12).all()
            and (q50 <= upper + 1e-12).all()
            and (upper <= 1.0 + 1e-12).all()
        ),
        ...
        "transport_boundary_semantics_pass": (
            boundary_semantics_pass
        ),
        "fallback_used_n": int(
            frame[
                "fallback_used"
...
def generate_historical_path(
    ...
    wapp_bridge: pd.DataFrame,
    perception_seed: int,
) -> pd.DataFrame:
    """
    Exact historical 0C-3C RNG consumption: one continuous rng per seed.
    """
    ...
    for day_index, r in wapp_bridge.iterrows():
...
def compare_historical_to_frozen_bank(
    generated: pd.DataFrame,
    frozen_bank: pd.DataFrame,
) -> tuple[pd.DataFrame, bool]:
    """
    Stronger v1.1 gate: historical generation must reproduce the frozen
    P2-0C-3C trajectory bank row-for-row under all six seeds.
    """
    rows = []
    all_pass = True
    ...
    for seed in PERCEPTION_SEEDS:
        ...
        if len(g) != len(b):
            raise RuntimeError(
                f"Historical bank row mismatch for seed {seed}."
            )

        exact_checks = {
            "date": bool(
                np.array_equal(
                    g["date"].astype(str).to_numpy(),
                    b["date"].astype(str).to_numpy(),
                )
            ),
            "source_sample_id": bool(
                np.array_equal(
                    g["source_sample_id"].astype(str).to_numpy(),
                    b["source_sample_id"].astype(str).to_numpy(),
                )
            ),
            "source_date": bool(
                np.array_equal(
                    g["source_date"].astype(str).to_numpy(),
                    b["source_date"].astype(str).to_numpy(),
                )
            ),
            "source_block": bool(
                np.array_equal(
                    g["source_block_id"].astype(str).to_numpy(),
                    b["source_block10_id"].astype(str).to_numpy(),
                )
            ),
            "lower_clipped": bool(
                np.array_equal(
                    g["lower_clipped"].astype(bool).to_numpy(),
                    b["lower_clipped"].astype(bool).to_numpy(),
                )
            ),
            "source_lower_clipped": bool(
                np.array_equal(
                    g["source_lower_clipped"].astype(bool).to_numpy(),
                    b["source_lower_clipped"].astype(bool).to_numpy(),
                )
            ),
            "candidate_samples": bool(
                np.array_equal(
                    g["candidate_samples"].astype(int).to_numpy(),
                    b["candidate_samples"].astype(int).to_numpy(),
                )
            ),
            "candidate_dates": bool(
                np.array_equal(
                    g["candidate_dates"].astype(int).to_numpy(),
                    b["candidate_dates"].astype(int).to_numpy(),
                )
            ),
            "candidate_blocks": bool(
                np.array_equal(
                    g["candidate_blocks"].astype(int).to_numpy(),
                    b["candidate_blocks"].astype(int).to_numpy(),
                )
            ),
        }
        ...
        mapping = {
            ...
            "upper": "upper",
            "width": "width",
            "source_true_L": "source_true_L",
        }
        for gc, bc in mapping.items():
            ga = pd.to_numeric(
                g[gc], errors="raise"
            ).to_numpy(dtype=float)
            ba = pd.to_numeric(
        ...
        rows.append(
            {
                ...
                "exact_pass": seed_pass,
                "exact_columns": json.dumps(
                    exact_checks,
                    sort_keys=True,
                ),
                "numeric_columns": json.dumps(
                    numeric_checks,
                    sort_keys=True,
                ),
                "max_abs_diff": json.dumps(
                    max_abs_diff,
                    sort_keys=True,
                ),
            }
...
def perception_substream_seed(
    ...
    """
    Deterministic independent substream per (seed, year, trajectory, day).

    Threshold/policy is deliberately absent. Therefore policy-dependent
    candidate sets cannot desynchronize future days' perception randomness.
    """
...
def audit_online_perception_subset(
    ...
    for seed in PERCEPTION_SEEDS:
        ...
        m.update(
            {
                ...
                "point_ua_q50_pairing_pass": bool(
                    f[
                        "point_q50_equals_ua_q50"
                    ].all()
                ),
                "seasonal_pairing_pass": bool(
                    f[
                        "seasonal_features_identical"
                    ].all()
                ),
                "query_L_distribution": qstats(
                    f["query_L"]
                ),
            }
        )
        ...
        # Small manual-review trace only.
        keep_n = min(
            100,
            len(f),
        )
...
def main() -> int:
    ...
    p.add_argument("--frozen-trajectory-bank", required=True, type=Path)
    ...
    p.add_argument(
        ...
        default=Path(
            ...
            "p2_1c2b_online_counterfactual_perception_audit_v1_1"
        ),
    )
    args = p.parse_args()
    ...
    paths = {
        ...
        "wapp_power_bridge": args.wapp_power_bridge.expanduser().resolve(),
        "frozen_trajectory_bank": args.frozen_trajectory_bank.expanduser().resolve(),
    ...
    wapp_bridge = load_wapp_power_bridge(paths["wapp_power_bridge"])
    frozen_bank = load_frozen_trajectory_bank(paths["frozen_trajectory_bank"])
    frozen_seed_metrics = load_frozen_seed_metrics(
        paths["frozen_seed_metrics"]
    )

    actual_hashes = {
        "paper1_cqr": sha256_file(paths["paper1_cqr"]),
        "wapp_power_bridge": sha256_file(paths["wapp_power_bridge"]),
        "frozen_emulator_script": sha256_file(paths["frozen_emulator_script"]),
        "frozen_trajectory_bank": sha256_file(paths["frozen_trajectory_bank"]),
        "frozen_seed_metrics": sha256_file(paths["frozen_seed_metrics"]),
    }
    provenance_hash_pass = bool(
        all(
            actual_hashes[k] == EXPECTED_HASHES[k]
            for k in EXPECTED_HASHES
        )
    )
    if not provenance_hash_pass:
        raise RuntimeError(
            "Frozen perception provenance hash drift. "
            "Do not regenerate or retune assets."
        )
    ...
    ledger_valid = ledger.loc[
        ledger["state_valid"]
        & np.isfinite(ledger["L_power_proxy"]),
        ["date", "L_power_proxy"],
    ].copy().reset_index(drop=True)
    ledger_valid["date_string"] = (
        pd.to_datetime(
            ledger_valid["date"],
            errors="raise",
        ).dt.strftime("%Y-%m-%d")
    )
    ledger_wapp_bridge_alignment_pass = bool(
        len(ledger_valid) == len(wapp_bridge)
        and np.array_equal(
            ledger_valid["date_string"].to_numpy(),
            wapp_bridge["date"].astype(str).to_numpy(),
        )
        and np.allclose(
            ledger_valid["L_power_proxy"].to_numpy(dtype=float),
            wapp_bridge["L_power_proxy"].to_numpy(dtype=float),
            atol=1e-15,
            rtol=0.0,
        )
    )
    if not ledger_wapp_bridge_alignment_pass:
        raise RuntimeError(
            "Master ledger valid hidden states do not exactly align with "
            "the frozen WAPP bridge used by P2-0C-3C."
        )

    print("[4/12] Build BLOCK10 online emulator and historical support index")
    emulator = OnlineBlock10Emulator(source)
    historical_queries = wapp_bridge["L_power_proxy"].to_numpy(dtype=float)
    ...
    historical_bank_exact_df, historical_bank_exact_pass = (
        compare_historical_to_frozen_bank(
    ...
    source_asset_gates = {
        ...
        "frozen_provenance_hash_pass": provenance_hash_pass,
        "ledger_WAPP_bridge_alignment_pass": ledger_wapp_bridge_alignment_pass,
    }

    historical_gates = {
        "historical_WAPP_support_100pct_pass": historical_support_100,
        "historical_exact_frozen_bank_reproduction_pass": (
            historical_bank_exact_pass
        ),
    ...
    all_source_asset_gates = bool(
        ...
        and source_asset_gates["frozen_provenance_hash_pass"]
        and source_asset_gates["ledger_WAPP_bridge_alignment_pass"]
    )
    ...
    audit_summary = {
        ...
        "implementation_version": "v1.1_R1_VERIFIED_FROZEN_SAMPLER",
        "reward_built": False,
        "gym_built": False,
        "rl_started": False,
        ...
        "frozen_perception_design": {
            ...
            "selector": "EXACT_FROZEN_BLOCK10_RNG_CHOICE_THEN_GAUSSIAN_RNG_CHOICE",
            ...
            "fallback": False,
            "original_paper1_csv_row_order_preserved": True,
            "non_source_clipped_lower_may_clip_after_transport": True,
            "R1_implementation_correction_only": True,
            "wapp_irradiance_used": False,
        ...
        "provenance": {
            ...
            "paper1_cqr_sha256": actual_hashes["paper1_cqr"],
            "wapp_power_bridge_path": str(paths["wapp_power_bridge"]),
            "wapp_power_bridge_sha256": actual_hashes["wapp_power_bridge"],
            "frozen_trajectory_bank_path": str(paths["frozen_trajectory_bank"]),
            "frozen_trajectory_bank_sha256": actual_hashes["frozen_trajectory_bank"],
            "frozen_0C3C_script_path": str(
                paths["frozen_emulator_script"]
            ),
            "frozen_0C3C_script_sha256": actual_hashes["frozen_emulator_script"],
            "frozen_seed_metrics_path": str(
                paths["frozen_seed_metrics"]
            ),
            "frozen_seed_metrics_sha256": actual_hashes["frozen_seed_metrics"],
        },
        "randomness_contract": {
            ...
            "perception_substream_rule": "(seed,year,trajectory,day); policy excluded",
            "perception_substreams_reused_across_probe_policies": True,
        },
        "observation_schema": {
        ...
        "historical_exact_frozen_bank_regression": (
            historical_bank_exact_df.to_dict(orient="records")
        ),
    ...
    historical_bank_exact_df.to_csv(
        out / "historical_exact_frozen_bank_regression.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    print(out / "historical_exact_frozen_bank_regression.csv")
