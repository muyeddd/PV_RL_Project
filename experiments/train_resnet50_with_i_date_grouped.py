"""Leakage-free date-grouped ResNet50+I training entry point."""
...
DEFAULT_CONFIG_PATH = (
    PROJECT_ROOT / "configs" / "training" / "resnet50_with_i_date_grouped_v1.json"
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train ResNet50+I using the frozen date_grouped_v1 manifest"
    )
...
def unpack_batch(batch):
    if len(batch) < 3:
        raise ValueError("Expected image, loss label, and irradiance")
